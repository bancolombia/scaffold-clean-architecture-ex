defmodule ApplyTemplatesTest do
  ...
  test "should get variable list" do
    ...
    res = ApplyTemplate.manage_application_name(project_name)
    assert {:ok, _project_name, "TestProject"} = res
  end

  test "should get invalid appliation name" do
    res = ApplyTemplate.manage_application_name("invalidname")
    assert {:error, :invalid_application_name} = res
  end

  test "should get variables list" do
    res = ApplyTemplate.create_variables_list(:test, "test")
    assert {:ok, [
  ...
  test "should load template file" do
      res = ApplyTemplate.load_template_file("test/assets/create_structure.exs")
  ...
  test "should create directories and files" do
    with_mock File, [mkdir_p: fn(_path) -> :ok end] do
      res = ApplyTemplate.create_content("/path")
      assert :ok == res
    end
  end
  ...
  test "should create files" do
    ...
    with_mocks([
      ...
      res = ApplyTemplate.create_folder(template, "test_folder", variable_list)
      assert :ok == res
